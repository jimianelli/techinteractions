...
##############################################################################	
	...
	Select_data <- as.data.frame(cbind(ALL_clust[,c(1:3, ncol(ALL_clust))], Select_data))
	Select_data$Gear <- factor(Select_data$Gear, labels=c("non pelagic trawl", "pelagic trawl", "trap/pot", "longline"))
	BSAI_data <- subset(Select_data, Area %in% BSAI)
	...
	BSAI_data_partial <- as.data.frame(cbind(BSAI_data[,1:4], t(apply(BSAI_data[,-c(1:4)], 1, function(x) x/sum(x)))))
	...
	to_keep <- which(BSAI_data_partial$PACIFIC.HALIBUT<0.5)
...
{	
	...
	Data_input_fake <- t(apply(Data_input_true, 1, function(x) { aaa = (x+runif(4,0,0.5)); bbb = aaa/sum(aaa); return(bbb)}))
	## Choosing the data to use
	Data_input <- Data_input_true
	...
	write("# Number of fishing strategy", file="main_code.dat")
	write(Nb_strategy, file="main_code.dat", append=T)
	# Number of species
	write("# Number of species (including bycatch sp)", file="main_code.dat", append=T)
	write(Nb_species, file="main_code.dat", append=T)
	# Price of species
	write("# Price species (including bycatch sp)", file="main_code.dat", append=T)
	write(price, file="main_code.dat", append=T)
	# Catch proportion matrix 
	write("# Catch proportion by fishing strategy", file="main_code.dat", append=T)
	write(Data_input, file="main_code.dat", ncolumns = Nb_strategy, append=T)
	# Total Number of b1 constaints
	write("# Total number of b1 constraints", file="main_code.dat", append=T)
	write(sum(Nb_constraints_b1), file="main_code.dat", append=T)
	# Total Number of b2 constaints
	write("# Total number of b2 constraints", file="main_code.dat", append=T)
	write(sum(Nb_constraints_b2), file="main_code.dat", append=T)
	# Number of types of b1 constraints 
	write("# Number of types of b1 constraints", file="main_code.dat", append=T)
	write(length(Nb_constraints_b1), file="main_code.dat", append=T)
	# Number of each type of b1 constraints 
	write("# Number of each type of b1 constraints", file="main_code.dat", append=T)
	write(Nb_constraints_b1, file="main_code.dat", append=T)
	# Number of types of b2 constraints 
	write("# Number of types of b2 constraints", file="main_code.dat", append=T)
	write(length(Nb_constraints_b2), file="main_code.dat", append=T)
	# Number of each type of b2 constraints 
	write("# Number of each type of b2 constraints", file="main_code.dat", append=T)
	write(Nb_constraints_b2, file="main_code.dat", append=T)
	# The a0 vector = the objective function
	write("# the a0 vector = objective function", file="main_code.dat", append=T)
	write(obj_fun, file="main_code.dat", ncolumns = Nb_strategy, append=T)
	# the a1 matrix
	write("# The A1 matrix", file="main_code.dat", append=T)
	write((Data_input), file="main_code.dat", ncolumns = Nb_strategy, append=T)			# constraints about the ABC
	write(rep(1,Nb_strategy), file="main_code.dat", ncolumns = Nb_strategy, append=T)	# constraint about OY
	write(D_upper, file="main_code.dat", ncolumns = Nb_strategy, append=T)				# constraint on the "dK' upper bound
	# the a2 matrix
	write("# The A2 matrix", file="main_code.dat", append=T)
	write((D_lower), file="main_code.dat", ncolumns = Nb_strategy, append=T)			# constraint on the "dK' lower bound
	# the initial weight = "dk" values
	write("# The initial weight = dk values", file="main_code.dat", append=T)
	write(Data_weigthing, file="main_code.dat", ncolumns = Nb_strategy, append=T)			# constraint on the "dK' lower bound
	# the b1 bounds (the upper bounds value of each b1 constraints)
	write("# The b1 bounds ", file="main_code.dat", append=T)
	write(Bounds_b1, file="main_code.dat", ncolumns = Nb_strategy, append=T)			# constraint on the "dK' lower bound
	# the b2 bounds (the upper bounds value of each b2 constraints)
	write("# The b2 bounds ", file="main_code.dat", append=T)
	write(Bounds_b2, file="main_code.dat", ncolumns = Nb_strategy, append=T)			# constraint on the "dK' lower bound
