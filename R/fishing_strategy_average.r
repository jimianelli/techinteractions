#' This file controls the input values that goes into the quota allocation and vessel dynamics model and run them. This is a static version (no dynamic change in net price nor changes in catch composition by metier over time) of "fishing_strategy.r" only used for model behavior checking. 
#'
...
#' @export
	...
	{	
		...
		# Number fishing strategies (main_code_average.dat)
	...
	Fishing_strategy(Choose_fish_strategies = "Average", Bounds_base = "cluster", Flex_adj = 1, Change_strategy=FALSE, seed=seed_val, price_change = FALSE, price_sd=FALSE)
